...
9. Make example predictions
   <!-- - During inference, multiple trajectories are generated by repeatedly sampling parameters — specifically, means and standard deviations — from the predicted mixture model in an autoregressive manner, in order to create a predictive distribution.
   ...
   - For visualisation, a density heatmap is created that shows the density of predicted positions as well as the final most likely position on a geographic map. -->
